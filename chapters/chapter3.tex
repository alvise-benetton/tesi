...
\begin{enumerate}
    \item \textbf{Dataset}: caricamento dei log di assemblaggio ed estrazione delle feature nelle due configurazioni \texttt{no\_human} e \texttt{human}, con partizionamento in 10 fold per la Cross-Validation.
...
\item \textbf{Addestramento e ottimizzazione}: addestramento dei quattro classificatori (Decision Tree, Random Forest, XGBoost e LSTM) mediante Nested Cross-Validation e ottimizzazione bayesiana degli iperparametri (Hyperopt TPE) orientata alla massimizzazione del coefficiente MCC sul validation set.
...
\begin{figure}[htbp]
    \centering
    \input{images/source/encoding_strategies.tex}
...
\begin{figure}[htbp]
    \centering
    \input{images/source/tabular_format.tex}
...
\begin{figure}[htbp]
    \centering
    \input{images/source/tensor_format.tex}
